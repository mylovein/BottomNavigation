...
public class BottomNavigationBehavior<V extends View> extends VerticalScrollingBehavior<V> {
	...
	public BottomNavigationBehavior(int bottomNavigationHeight,boolean behaviorTranslationEnabled) {
		super();
		this.behaviorTranslationEnabled = behaviorTranslationEnabled;

        mBottomNavigationHeight = bottomNavigationHeight;
	}
...
//		a.recycle();
	...
	private void handleDirection(V child, int scrollDirection) {
		...
		if (scrollDirection == ScrollDirection.SCROLL_DIRECTION_DOWN && hidden) {
			...
			hidden = true;
			animateOffset(child, mBottomNavigationHeight, false, true);
		}
	}
	...
	public void updateSnackbar(final View child, View dependency) {
		...
		if (dependency != null && dependency instanceof Snackbar.SnackbarLayout) {
				...
				snackbarLayout.addOnLayoutChangeListener(new View.OnLayoutChangeListener() {
					...
					public void onLayoutChange(View v, int left, int top, int right, int bottom,
						...
						if (floatingActionButton != null &&
                            ...
                            //只是删掉了snackBarY,虽然不知道为什么，好神奇！
							fabTargetOffset = fabDefaultBottomMargin - child.getTranslationY();
							p.setMargins(p.leftMargin, p.topMargin, p.rightMargin, (int) fabTargetOffset);
							floatingActionButton.requestLayout();
						}
					}
			...
			int targetMargin = (int) (mBottomNavigationHeight - child.getTranslationY());
